...
module.exports = util.Class(function(){
	return {
		 ...
		 * @return {[type]}          [description]
		 */
		upload: function(savePath, filePath, fileSecret){
			var headers = {
				mkdir: true
			};
			if (fileSecret) {
				headers["Content-Secret"] = fileSecret;
			};
			var self = this;
			//文件上传
			if (util.isFile(filePath)) {
				var stream = fs.readFileSync(filePath);
				return this.request(savePath, 'PUT', stream, headers).then(function(response){
				...
				return this.request(savePath, 'PUT', filePath, headers).then(function(response){
					return self.getPicInfo(response);
				});
			}
		},
